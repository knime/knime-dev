...
/**
 * Checker for {@link LocalDateTime} which allows to truncate time units.
 *
 * @author Tobias Koetter, KNIME GmbH, Konstanz, Germany
 * @since 3.8
 */
public class LocalDateTimeChecker extends AbstractDifferenceChecker<LocalDateTimeValue> {

    private static final ChronoUnit DEFAULT_UNIT = ChronoUnit.MILLIS;

    /**
     * Factory for the {@link LocalDateTimeChecker}.
     */
    public static class Factory implements DifferenceCheckerFactory<LocalDateTimeValue> {
        /**
         * {@inheritDoc}
         */
        @Override
        public Class<LocalDateTimeValue> getType() {
         ...
         */
        @Override
        public DifferenceChecker<LocalDateTimeValue> newChecker() {
            return new LocalDateTimeChecker();
        }
    }

    static final String DESCRIPTION = "Truncated Local Date Time";
     ...
     */
    @Override
    public Result check(final LocalDateTimeValue expected, final LocalDateTimeValue got) {
        final LocalDateTime ev = expected.getLocalDateTime();
        final LocalDateTime gv = got.getLocalDateTime();

        if (ev.equals(gv)) {
            return OK;
        } else {
            final ChronoUnit chronoUnit = ChronoUnit.valueOf(m_unit.getStringValue());
            final LocalDateTime truncev = ev.truncatedTo(chronoUnit);
            final LocalDateTime truncgv = gv.truncatedTo(chronoUnit);
